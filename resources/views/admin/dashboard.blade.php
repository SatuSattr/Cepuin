...
<body class="antialiased ">
    ...
    <div class="min-h-screen lg:flex">
        <aside id="sidebar"
            class="fixed inset-y-0 left-0 z-40 w-72 transform bg-[var(--color-secondary)] text-white shadow-xl transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:shadow-none">
                            ...
                            <a href="{{ route('admin.studentdata') }}"
                                ...
                                <span class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white/20">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                            d="M17 20h5V4a1 1 0 00-1-1H3a1 1 0 00-1 1v16h5" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                            d="M7 20v-6h10v6m-7-6v-3a2 2 0 014 0v3" />
                                    </svg>
                                </span>
                                Data Siswa
                            </a>
                ...
                <div class="mt-auto px-6 pb-8 pt-6">
                    ...
                    <div class="mt-6 rounded-3xl border border-white/10 bg-white/5 p-5">
                        ...
                        <div class="mt-3 flex items-center gap-3 text-sm text-white/80">
                            <span
                                class="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-white/15 text-base font-semibold">
                                {{ strtoupper(substr(optional(auth()->user())->name ?? 'A', 0, 1)) }}
                            </span>
                            <div>
                                <p class="font-semibold text-white">{{ optional(auth()->user())->name ?? 'Admin' }}</p>
                                <p class="text-xs text-white/60">
                                    {{ optional(auth()->user())->email ?? 'admin@example.com' }}</p>
                            </div>
                        </div>
                            ...
                            <button type="submit"
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20"
                                    ...
                                    <path fill-rule="evenodd"
                                        d="M3 4.5A1.5 1.5 0 014.5 3h6a1.5 1.5 0 011.5 1.5V7a.75.75 0 01-1.5 0V4.5h-6v11h6V13a.75.75 0 011.5 0v2.5a1.5 1.5 0 01-1.5 1.5h-6A1.5 1.5 0 013 15.5v-11z"
                                        clip-rule="evenodd" />
                                    <path
                                        d="M12.22 9.22a.75.75 0 010 1.06l-2.47 2.47a.75.75 0 101.06 1.06l1.97-1.97 1.97 1.97a.75.75 0 101.06-1.06l-2.47-2.47a.75.75 0 010-1.06l2.47-2.47a.75.75 0 10-1.06-1.06L12.81 7.5l-1.97-1.97a.75.75 0 10-1.06 1.06l2.44 2.63z" />
                                </svg>
                                Keluar
                            </button>
                        </form>
        ...
        <div
            class="relative flex min-h-screen flex-1 flex-col bg-[var(--color-secondary)] lg:ml-30 lg:h-screen lg:overflow-y-auto">
            <div class="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
                <div
                    class="absolute -top-24 -left-36 hidden h-72 w-72 rounded-full bg-[var(--color-secondary)]/12 blur-3xl lg:block">
                </div>
                <div
                    class="absolute -bottom-32 right-[-6rem] hidden h-[26rem] w-[26rem] rounded-full bg-[var(--color-secondary)]/12 blur-3xl md:block">
                </div>
            </div>
            ...
            <div class="relative z-10 w-full px-4 py-8 sm:px-6 lg:px-8">
                <div class="mx-auto flex w-full  flex-col gap-8">
                    <section
                        class="rounded-[40px] border border-white/60 bg-primary px-6 py-6 shadow-[0_40px_80px_-48px_rgba(128,1,1,0.55)] backdrop-blur sm:px-10 sm:py-[5rem]">
                        <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                            <div>
                                <p class="text-sm font-medium text-neutral-400">Halo,
                                    {{ optional(auth()->user())->name ?? 'Admin' }}</p>
                                <h1 class="mt-2 text-3xl font-semibold text-[var(--color-secondary)]">Panel Pengaduan BK
                                </h1>
                                <p class="mt-3 max-w-xl text-sm text-neutral-500">
                                    Kelola laporan siswa dengan satu tempat terpadu. Pantau status, perbarui catatan
                                    konselor, dan lakukan tindak lanjut secara cepat.
                                </p>
                            </div>
                            <button
                        ...
                        @if (session('success'))
                            <div
                                class="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 shadow">
                                {{ session('success') }}
                            </div>
                        @endif
                        ...
                        <div class="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.75fr)_minmax(0,0.9fr)]">
                            <div class="flex flex-col gap-6">
                                <div
                                    ...
                                    <div>
                                        <h2 class="text-lg font-semibold text-[var(--color-secondary)]">Semua Laporan
                                        </h2>
                                        <p class="text-xs text-neutral-500">Tinjau laporan terbaru dan lakukan tindakan
                                            yang dibutuhkan.</p>
                                    </div>
                                    <form method="GET" class="flex flex-col gap-2 sm:flex-row sm:items-center">
                                        <label for="status"
                                            class="text-xs font-medium text-neutral-500 sm:mr-2">Filter status</label>
                                        <div class="flex items-center gap-2">
                                            <select id="status" name="status"
                                                class="rounded-xl border border-neutral-300 bg-white px-3 py-2 text-xs font-medium text-neutral-600 shadow-sm focus:border-[var(--color-secondary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-secondary)]/30">
                                            ...
                                            <button type="submit"
                                                class="inline-flex items-center rounded-xl bg-[var(--color-secondary)] px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-[#5c0101] focus:outline-none focus:ring-2 focus:ring-[var(--color-secondary)]/40">
                                                Terapkan
                                            </button>
                                            @if ($statusFilter)
                                                <a href="{{ route('admin.dashboard') }}"
                                                    class="inline-flex items-center rounded-xl border border-neutral-300 px-3 py-2 text-xs font-medium text-neutral-500 shadow-sm transition hover:border-neutral-400 hover:text-neutral-800">
                                                    Reset
                                                </a>
                                            @endif
                                        </div>
                                ...
                                <div
                                    class="overflow-hidden rounded-[32px] border border-neutral-100 bg-white shadow-[0_24px_60px_-45px_rgba(128,1,1,0.55)]">
                                    <div class="overflow-x-auto">
                                        <table class="min-w-full border-separate border-spacing-y-3 bg-white">
                                            <thead>
                                                <tr
                                                    class="text-left text-xs font-semibold uppercase tracking-[2px] text-neutral-500">
                                            ...
                                            <tbody class="text-sm text-neutral-700">
                                                @forelse ($reports as $report)
                                                    <tr
                                                        class="rounded-2xl bg-white shadow-[0_20px_50px_-35px_rgba(128,1,1,0.5)]">
                                                        <td class="px-6 py-4 align-top">
                                                            <div>
                                                                <p class="font-semibold text-neutral-900">
                                                                    {{ $report->user->name ?? 'User #' . $report->user_id }}
                                                                </p>
                                                                <p class="text-xs text-neutral-500">ID
                                                                    {{ $report->user_id }}</p>
                                                            </div>
                                                        </td>
                                                        <td class="px-6 py-4 align-top">
                                                            <p class="font-medium text-neutral-900">
                                                                {{ $report->reported_name }}</p>
                                                        </td>
                                                        <td class="px-6 py-4 align-top">
                                                            <span
                                                                class="inline-flex rounded-xl bg-neutral-100 px-3 py-1 text-xs font-medium text-neutral-600">{{ $report->reported_class }}</span>
                                                        </td>
                                                        <td class="px-6 py-4 align-top">
                                                            @php
                                                                $incidentAt = $report->incident_time
                                                        ...
                                                        <td class="px-6 py-4 align-top text-right">
                                                            <button type="button"
                                                                ...
                                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                                    ...
                                                                    fill="currentColor">
                                                                    <path
                                                                        d="M2.94 10.94l-.59-.59a1.5 1.5 0 010-2.12l3.82-3.82a1.5 1.5 0 012.12 0l1.17 1.17-5.94 5.94a1 1 0 01-1.41 0z" />
                                                                    <path
                                                                        d="M6.59 14.59l5.94-5.94 1.17 1.17a1.5 1.5 0 010 2.12l-3.82 3.82a1.5 1.5 0 01-2.12 0l-1.17-1.17z" />
                                                                    <path
                                                                        d="M12.71 3.29a1 1 0 011.41 0l2.59 2.59a1 1 0 010 1.41l-1.88 1.88-3.99-3.99 1.87-1.89z" />
                                                                </svg>
                                                            </button>
                                                        </td>
                                                    </tr>
                                                @empty
                                                    <tr>
                                                        <td colspan="6"
                                                            class="px-6 py-8 text-center text-sm text-neutral-500">
                            ...
                            <aside class="space-y-6">
                                <div
                                    class="rounded-[32px] bg-gradient-to-br from-[var(--color-secondary)] to-[#470000] p-6 text-white shadow-[0_30px_60px_-45px_rgba(71,0,0,0.8)]">
                                    ...
                                    <div class="mt-6 grid gap-3">
                                        @foreach ($statusLabels as $key => $label)
                                            <div class="flex items-center justify-between">
                                                <span
                                                    class="text-xs font-medium text-white/80">{{ $label }}</span>
                                                <span
                                                    class="text-sm font-semibold text-white">{{ $summaryCounts[$key] }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                                ...
                                <div
                                    class="rounded-[32px] border border-white/70 bg-white/80 p-6 shadow-[0_20px_50px_-35px_rgba(128,1,1,0.4)]">
                                    <p class="text-xs font-semibold uppercase tracking-[3px] text-neutral-400">Catatan
                                        Cepat</p>
                                    <div class="mt-4 space-y-4">
                                        <div
                                            class="rounded-2xl bg-[var(--color-primary)] px-4 py-3 text-sm text-neutral-600 shadow-sm">
                                            Pastikan setiap laporan minimal mendapat status <strong
                                                class="font-semibold text-[var(--color-secondary)]">Direview</strong>
                                            dalam 24 jam.
                                        </div>
                                        <div
                                            class="rounded-2xl bg-[var(--color-primary)] px-4 py-3 text-sm text-neutral-600 shadow-sm">
                                            Gunakan kolom catatan untuk menyimpan ringkasan konseling atau tindak lanjut
                                            ke wali kelas.
                                        </div>
                                    </div>
                                </div>
            ...
            <div data-modal-panel class="relative w-full max-w-3xl rounded-3xl bg-white shadow-2xl">
                <div class="flex items-start justify-between border-b border-neutral-100 px-6 py-5">
                    <div>
                        ...
                        <h3 class="text-xl font-semibold text-[var(--color-secondary)]">{{ $report->reported_name }}
                        </h3>
                    </div>
                    <button
                        class="rounded-full bg-neutral-100 p-2 text-neutral-500 transition hover:bg-neutral-200 hover:text-neutral-700 focus:outline-none focus:ring-2 focus:ring-[var(--color-secondary)]/30"
                        data-modal-close>
                ...
                <div class="grid gap-6 px-6 py-6 lg:grid-cols-[1.2fr_1fr]">
                    <div class="space-y-6">
                        <section class="rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-4">
                            ...
                            <dl class="mt-4 grid gap-3 text-sm text-neutral-600">
                                <div class="flex justify-between gap-4">
                                    ...
                                    <dd class="text-right">{{ $report->user->name ?? 'User #' . $report->user_id }}
                                    </dd>
                                </div>
                                <div class="flex justify-between gap-4">
                                ...
                                <div class="flex justify-between gap-4">
                                    ...
                                    <dd class="text-right">
                                        @php
                                            $incidentAt = $report->incident_time
                                                ? \Illuminate\Support\Carbon::parse($report->incident_time)
                                                : null;
                                        @endphp
                        ...
                        <section>
                            ...
                            <div
                                class="mt-3 rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm leading-relaxed text-neutral-700 shadow-sm">
                                {{ $report->description }}
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <form method="POST" action="{{ route('admin.reports.update', $report) }}"
                                ...
                                @csrf
                                @method('PUT')
                                <textarea name="counselor_note" rows="4"
                                    ...
                                    placeholder="Tambahkan catatan tindak lanjut konselor...">{{ old('counselor_note', $report->counselor_note) }}</textarea>
                            ...
                            <form method="POST" action="{{ route('admin.reports.destroy', $report) }}"
                                ...
                                <button type="submit"
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                            d="M6 7h12m-9 0v10m6-10v10M9 3h6a1 1 0 011 1v2H8V4a1 1 0 011-1z" />
                                    </svg>
                                    Hapus Laporan
                                </button>
                            </form>
